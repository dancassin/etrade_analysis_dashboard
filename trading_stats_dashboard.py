...
tax_lot_df["pct_gain/loss"] = round(
    (tax_lot_df["gain"] / tax_lot_df["total_cost"]) * 100, 2
)

tax_lot_df["category"] = pd.cut(
    tax_lot_df.gain, bins=[-5000, -0.000001, 5000], labels=["loss", "gain"]
)
...
def avg_gain(jsonified_cleaned_data):
    ...
    fig.update_layout(
        xaxis_title_text="% Gain/Loss",  # xaxis label
    ...
    fig.add_annotation(
        text=f"Max Gain: <br>{round(max(gain_df['pct_gain/loss']))}%",
        xref="paper",
        yref="paper",
        x=1,
        y=-0.35,
    ...
    fig.add_annotation(
        text=f"Max Loss: <br>{round(min(loss_df['pct_gain/loss']))}%",
        xref="paper",
        yref="paper",
        x=0,
        y=-0.35,
...
@app.callback(Output("equity", "figure"), [Input("sub_df", "data")])
...
@app.callback(
    [
        Output("risk_reward_num", "children"),
...
def update_reward_risk(jsonified_cleaned_data):
    ...
    reward_risk = round(
        (total_winning_trades * avg_amount_gain)
        / (total_losing_trades * avg_amount_loss),
        2,
    )
    ...
    expectancy = (
        round(
            ((pct_winning_trades * avg_pct_gain) - (pct_losing_trades * avg_pct_loss)),
            4,
        )
        / 100
    )
    ...
    frequency = round((df.shape[0] * 0.8), 2)
